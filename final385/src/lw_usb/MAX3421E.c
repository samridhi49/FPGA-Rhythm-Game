...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int Status = XSpi_Transfer(&SpiInstance, write, NULL, 2);

	if (Status != XST_SUCCESS)
	{
		xil_printf ("SPI write failed %d\n", Status);
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int Status = XSpi_Transfer(&SpiInstance, write, NULL, 1 + nbytes);

	if (Status != XST_SUCCESS)
	{
		xil_printf ("SPI multiple byte write failed %d\n", Status);
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	int Status = XSpi_Transfer(&SpiInstance, write, read, 2);

	if (Status != XST_SUCCESS)
	{
		xil_printf ("SPI read failed %d\n", Status);
		XSpi_SetSlaveSelect(&SpiInstance, 0x00);
		return 0;
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int Status = XSpi_Transfer(&SpiInstance, write, read, 1 + nbytes);
